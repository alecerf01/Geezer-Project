...
$.ajax(deezerAPI).then(function (response) {
	...
	var title = $("<div>").addClass("song-name text-center").text("Song: " + response.data[0].title);
	
	var albumImage = $("<img>").addClass("album-image").attr("src", response.data[0].album.cover_big)
	.css({"border-radius": "10px", "height":"300px"});
	
	var artist = $("<div>").addClass("artist text-center").text("Artist: " + response.data[0].artist.name)

	// audio element

	var audioEl = $("<audio>").attr("controls","")
	
	var songSample = $("<source>").attr({"src": response.data[0].preview, "type": "audio/mpeg"})
